...
def test_store_bulk_transactions_raise_store_error(b, monkeypatch, backend_query):
    from bigchaindb.models import Transaction
    from bigchaindb.common.crypto import generate_key_pair
    alice = generate_key_pair()
    tx = Transaction.create([alice.public_key],
    ...
    def mock_backend_query(conn, data):
...
def test_store_bulk_transactions_raise_store_block_error(b, monkeypatch):
    from bigchaindb.models import Transaction
    from bigchaindb.common.crypto import generate_key_pair
    from bigchaindb.tendermint.lib import Block
    ...
    assert not list(backend.query.get_metadata(b.connection, [tx.id])) == []
    assert backend.query.get_asset(b.connection, tx.id)
    assert b.get_transaction(tx.id)

    block = Block(app_hash='random_app_hash', height=90,
                  transactions=[tx.id])._asdict()

    def mock_block_store(conn, block):
        raise Exception('Some weird error occured')

    monkeypatch.setattr(
        'bigchaindb.backend.query.store_block', mock_block_store)
