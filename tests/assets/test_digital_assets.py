...
def test_get_asset_id_create_transaction(merlin, user_pk):
    from bigchaindb.models import Transaction
    tx_create = Transaction.create([merlin.public_key], [([user_pk], 1)])
...
def test_asset_id_mismatch(merlin, user_pk):
    from bigchaindb.models import Transaction
    from bigchaindb.common.exceptions import AssetIdMismatch

    tx1 = Transaction.create([merlin.public_key], [([user_pk], 1)],
                             metadata={'msg': random.random()})
    tx1.sign([merlin.private_key])
    tx2 = Transaction.create([merlin.public_key], [([user_pk], 1)],
                             metadata={'msg': random.random()})
    tx2.sign([merlin.private_key])
