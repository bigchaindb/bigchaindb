...
# Single output
# Single owners_after
def test_single_in_single_own_single_out_single_own_create(merlin, user_pk, b):
    from bigchaindb.models import Transaction

    tx = Transaction.create([merlin.public_key], [([user_pk], 100)], asset={'name': random.random()})
    tx_signed = tx.sign([merlin.private_key])
...
def test_single_in_single_own_multiple_out_single_own_create(merlin, user_pk, b):
    from bigchaindb.models import Transaction

    tx = Transaction.create([merlin.public_key], [([user_pk], 50), ([user_pk], 50)],
...
def test_single_in_single_own_single_out_multiple_own_create(merlin, user_pk, b):
    from bigchaindb.models import Transaction

    tx = Transaction.create([merlin.public_key], [([user_pk, user_pk], 100)], asset={'name': random.random()})
    tx_signed = tx.sign([merlin.private_key])
...
#      owners_after
def test_single_in_single_own_multiple_out_mix_own_create(merlin, user_pk, b):
    from bigchaindb.models import Transaction

    tx = Transaction.create([merlin.public_key], [([user_pk], 50), ([user_pk, user_pk], 50)],
                            asset={'name': random.random()})
    tx_signed = tx.sign([merlin.private_key])
...
def test_single_in_multiple_own_single_out_single_own_create(merlin, b, user_pk,
                                                             user_sk):
    from bigchaindb.models import Transaction
    from bigchaindb.common.transaction import _fulfillment_to_details

    tx = Transaction.create([merlin.public_key, user_pk], [([user_pk], 100)], asset={'name': random.random()})
    tx_signed = tx.sign([merlin.private_key, user_sk])
    assert tx_signed.validate(b) == tx_signed
    assert len(tx_signed.outputs) == 1
    assert tx_signed.outputs[0].amount == 100
    assert len(tx_signed.inputs) == 1
...
# Single output
# Single owners_after
def test_single_in_single_own_single_out_single_own_transfer(merlin, b, user_pk,
                                                             user_sk):
    from bigchaindb.models import Transaction

    # CREATE divisible asset
    tx_create = Transaction.create([merlin.public_key], [([user_pk], 100)], asset={'name': random.random()})
    tx_create_signed = tx_create.sign([merlin.private_key])

    # TRANSFER
    tx_transfer = Transaction.transfer(tx_create.to_inputs(), [([merlin.public_key], 100)],
...
# Multiple output
# Single owners_after
def test_single_in_single_own_multiple_out_single_own_transfer(merlin, b, user_pk,
                                                               user_sk):
    from bigchaindb.models import Transaction
    ...
    tx_transfer = Transaction.transfer(tx_create.to_inputs(),
                                       [([merlin.public_key], 50), ([merlin.public_key], 50)],
                                       asset_id=tx_create.id)
    tx_transfer_signed = tx_transfer.sign([user_sk])
...
# Single output
# Multiple owners_after
def test_single_in_single_own_single_out_multiple_own_transfer(merlin, b, user_pk,
                                                               user_sk):
    from bigchaindb.models import Transaction
    ...
    # TRANSFER
    tx_transfer = Transaction.transfer(tx_create.to_inputs(),
                                       [([merlin.public_key, merlin.public_key], 100)],
                                       asset_id=tx_create.id)
    tx_transfer_signed = tx_transfer.sign([user_sk])
...
# Mix: one output with a single owners_after, one output with multiple
#      owners_after
def test_single_in_single_own_multiple_out_mix_own_transfer(merlin, b, user_pk,
                                                            user_sk):
    from bigchaindb.models import Transaction
    ...
    # TRANSFER
    tx_transfer = Transaction.transfer(tx_create.to_inputs(),
                                       [([merlin.public_key], 50), ([merlin.public_key, merlin.public_key], 50)],
                                       asset_id=tx_create.id)
    tx_transfer_signed = tx_transfer.sign([user_sk])
...
# Single output
# Single owners_after
def test_single_in_multiple_own_single_out_single_own_transfer(merlin, b, user_pk,
                                                               user_sk):
    from bigchaindb.models import Transaction
    from bigchaindb.common.transaction import _fulfillment_to_details

    # CREATE divisible asset
    tx_create = Transaction.create([merlin.public_key], [([merlin.public_key, user_pk], 100)],
                                   asset={'name': random.random()})
    tx_create_signed = tx_create.sign([merlin.private_key])
    ...
    tx_transfer = Transaction.transfer(tx_create.to_inputs(), [([merlin.public_key], 100)],
                                       asset_id=tx_create.id)
    tx_transfer_signed = tx_transfer.sign([merlin.private_key, user_sk])
...
# Single output
# Single owners_after
def test_multiple_in_single_own_single_out_single_own_transfer(merlin, b, user_pk,
                                                               user_sk):
    from bigchaindb.models import Transaction

    # CREATE divisible asset
    tx_create = Transaction.create([merlin.public_key], [([user_pk], 50), ([user_pk], 50)],
                                   asset={'name': random.random()})
    tx_create_signed = tx_create.sign([merlin.private_key])
...
# Single output
# Single owners_after
def test_multiple_in_multiple_own_single_out_single_own_transfer(merlin, b, user_pk,
                                                                 user_sk):
    from bigchaindb.models import Transaction
    from bigchaindb.common.transaction import _fulfillment_to_details
...
# Single output
# Single owners_after
def test_muiltiple_in_mix_own_multiple_out_single_own_transfer(merlin, b, user_pk,
                                                               user_sk):
    from bigchaindb.models import Transaction
    from bigchaindb.common.transaction import _fulfillment_to_details

    # CREATE divisible asset
    tx_create = Transaction.create([merlin.public_key], [([user_pk], 50), ([user_pk, merlin.public_key], 50)],
                                   asset={'name': random.random()})
    tx_create_signed = tx_create.sign([merlin.private_key])
...
# Mix: one output with a single owners_after, one output with multiple
#      owners_after
def test_muiltiple_in_mix_own_multiple_out_mix_own_transfer(merlin, b, user_pk,
                                                            user_sk):
    from bigchaindb.models import Transaction
    from bigchaindb.common.transaction import _fulfillment_to_details
    ...
    # TRANSFER
    tx_transfer = Transaction.transfer(tx_create.to_inputs(),
                                       [([merlin.public_key], 50), ([merlin.public_key, user_pk], 50)],
                                       asset_id=tx_create.id)
    tx_transfer_signed = tx_transfer.sign([merlin.private_key, user_sk])
...
# Single output
# Single owners_after
def test_multiple_in_different_transactions(merlin, b, user_pk, user_sk):
    from bigchaindb.models import Transaction

    # CREATE divisible asset
    # `b` creates a divisible asset and assigns 50 shares to `b` and
    # 50 shares to `user_pk`
    tx_create = Transaction.create([merlin.public_key], [([user_pk], 50), ([merlin.public_key], 50)],
                                   asset={'name': random.random()})
    tx_create_signed = tx_create.sign([merlin.private_key])
    ...
    tx_transfer1 = Transaction.transfer(tx_create.to_inputs([1]),
                                        [([user_pk], 50)],
                                        asset_id=tx_create.id)
    tx_transfer1_signed = tx_transfer1.sign([merlin.private_key])
...
def test_amount_error_transfer(merlin, b, user_pk, user_sk):
    from bigchaindb.models import Transaction
    ...
    tx_transfer = Transaction.transfer(tx_create.to_inputs(), [([merlin.public_key], 50)],
                                       asset_id=tx_create.id)
    tx_transfer_signed = tx_transfer.sign([user_sk])
    ...
    tx_transfer = Transaction.transfer(tx_create.to_inputs(), [([merlin.public_key], 101)],
                                       asset_id=tx_create.id)
    tx_transfer_signed = tx_transfer.sign([user_sk])
...
def test_threshold_same_public_key(merlin, b, user_pk, user_sk):
    ...
    # CREATE divisible asset
    tx_create = Transaction.create([merlin.public_key], [([user_pk, user_pk], 100)],
                                   asset={'name': random.random()})
    tx_create_signed = tx_create.sign([merlin.private_key])
...
def test_sum_amount(merlin, b, user_pk, user_sk):
    from bigchaindb.models import Transaction

    # CREATE divisible asset with 3 outputs with amount 1
    tx_create = Transaction.create([merlin.public_key], [([user_pk], 1), ([user_pk], 1), ([user_pk], 1)],
                                   asset={'name': random.random()})
    tx_create_signed = tx_create.sign([merlin.private_key])

    # create a transfer transaction with one output and check if the amount
    # is 3
    tx_transfer = Transaction.transfer(tx_create.to_inputs(), [([merlin.public_key], 3)],
                                       asset_id=tx_create.id)
    tx_transfer_signed = tx_transfer.sign([user_sk])
...
def test_divide(merlin, b, user_pk, user_sk):
    from bigchaindb.models import Transaction

    # CREATE divisible asset with 1 output with amount 3
    tx_create = Transaction.create([merlin.public_key], [([user_pk], 3)], asset={'name': random.random()})
    tx_create_signed = tx_create.sign([merlin.private_key])

    # create a transfer transaction with 3 outputs and check if the amount
    # of each output is 1
    tx_transfer = Transaction.transfer(tx_create.to_inputs(),
                                       [([merlin.public_key], 1), ([merlin.public_key], 1), ([merlin.public_key], 1)],
                                       asset_id=tx_create.id)
    tx_transfer_signed = tx_transfer.sign([user_sk])
