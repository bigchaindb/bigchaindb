...
class Dispatcher:
    """Dispatch events to websockets and rabbitmq.
    ...
    def publish(self):
        ...
        if config['rabbitmq']:
            mqConnection = pika.BlockingConnection(pika.ConnectionParameters(config['rabbitmq']['host'], config['rabbitmq']['port']))
        ...
        while True:
            ...
            if mqConnection:
                for str_item in str_buffer:
                    mqChannel.basic_publish(exchange='', routing_key=config['rabbitmq']['queue'], body=json.dumps(str_item))
                    print("----->========= ======================================= >>>>>>>>>>>");
                    print(json.dumps(str_item))
